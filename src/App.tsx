      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <Link to="/profile" className="flex items-center gap-3 px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
              <UserIcon size={16} />
              <span>Hồ sơ của tôi</span>
            </Link>
            <Link to="/settings" className="flex items-center gap-3 px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
              <Settings size={16} />
...
const DarkModeToggle = () => {
  ...
  return (
    <button
      ...
      className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-500 dark:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
...
const Navbar = ({ user, searchQuery, setSearchQuery, medalsCount }: { 
  ...
  return (
    <header className={cn(
      ...
      isScrolled ? "bg-white/80 backdrop-blur-lg shadow-md py-3 dark:bg-slate-950/80" : "bg-transparent"
    )}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {navLinks.map(link => (
            <Link 
              ...
              className={cn(
                ...
                location.pathname === link.path ? "text-emerald-600 bg-emerald-50 dark:bg-emerald-950/50 dark:text-emerald-400" : "text-slate-600 dark:text-slate-300 hover:text-emerald-500 dark:hover:text-emerald-400"
              )}
            >
              {link.icon}
              {link.name}
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          <div className="hidden xl:flex items-center bg-slate-100 dark:bg-slate-900 rounded-full px-4 py-1.5 border border-slate-200 dark:border-slate-800 focus-within:bg-white dark:focus-within:bg-slate-800 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
            ...
            <input 
              ...
              className="bg-transparent border-none focus:ring-0 text-sm ml-2 w-32 focus:w-48 transition-all outline-none dark:text-slate-200"
            />
          </div>

          <div className="flex items-center gap-2 sm:gap-4">
            {user && (
               <Link to="/rewards" className="flex items-center gap-2 px-3 py-1 bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-500 rounded-full text-xs font-bold border border-amber-100 dark:border-amber-900/50">
            ...
            <button 
              className="lg:hidden text-slate-900 dark:text-slate-100 p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 mt-4 overflow-hidden rounded-2xl shadow-xl"
          >
            <div className="p-6 flex flex-col gap-4">
              {navLinks.map(link => (
                <Link 
                  ...
                  className={cn(
                    ...
                    location.pathname === link.path ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 shadow-sm" : "text-slate-600 dark:text-slate-300 active:bg-slate-50 dark:active:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800"
                  )}
                >
              ...
              {!user && (
                <Link 
                  to="/login"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="mt-4 bg-emerald-600 dark:bg-emerald-500 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-3 text-lg"
                >
...
const LoginPage = ({ user }: { user: FirebaseUser | null }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center pt-20 px-6 bg-slate-50 dark:bg-slate-950 relative overflow-hidden">
      {/* Background Decor */}
      ...
      <motion.div 
        ...
        className="max-w-md w-full bg-white dark:bg-slate-900 rounded-[40px] shadow-3d p-10 relative z-10 border border-slate-100 dark:border-slate-800"
      >
        ...
        <div className="space-y-4">
          <button 
            onClick={handleLogin}
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-black py-4 px-6 border-2 border-slate-100 dark:border-slate-700 rounded-3xl transition-all active:scale-95 disabled:opacity-50 shadow-sm"
          >
            {isLoading ? (
          ...
          <div className="relative py-4">
            ...
            <div className="relative flex justify-center text-xs uppercase font-black tracking-widest text-slate-300">
              <span className="bg-white dark:bg-slate-900 px-4">HOẶC</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <input type="email" placeholder="email@example.com" className="w-full bg-slate-50 dark:bg-slate-800 dark:text-white border-none rounded-3xl p-4 focus:ring-2 focus:ring-emerald-500/20 transition-all outline-none" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 dark:text-slate-300 ml-4">Mật khẩu</label>
              <input type="password" placeholder="••••••••" className="w-full bg-slate-50 dark:bg-slate-800 dark:text-white border-none rounded-3xl p-4 focus:ring-2 focus:ring-emerald-500/20 transition-all outline-none" />
            </div>
          </div>
      ...
      <section className="relative pt-32 pb-20 px-6 overflow-hidden min-h-[90vh] flex items-center">
        ...
        <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          >
            ...
            <p className="text-xl text-slate-500 dark:text-slate-300 mb-10 max-w-xl leading-relaxed">
          ...
          >
            <div className="relative z-10 transform rotate-3 hover:rotate-0 transition-transform duration-700">
              ...
              />
              <div className="absolute -bottom-6 -left-6 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 animate-bounce-slow">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[
            ...
            >
              ...
              <h3 className="text-2xl font-black dark:text-white mb-4">{cat.title}</h3>
...
const ResourcesPage = () => {
  ...
  return (
    <div className="pt-20 flex min-h-screen bg-white dark:bg-slate-950">
      {/* Sidebar - Documentation Style */}
      <aside className="w-72 border-r border-slate-100 dark:border-slate-800 hidden lg:block sticky top-20 h-[calc(100vh-80px)] overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50">
        <div className="p-8">
          <h2 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-6">Danh mục tài liệu</h2>
      ...
      <main className="flex-1 p-8 md:p-12">
        <div className="max-w-4xl mx-auto">
          <div className="mb-12">
            <h1 className="text-4xl font-black text-slate-900 dark:text-white mb-4">{activeCategory}</h1>
            <p className="text-slate-500 dark:text-slate-400 font-medium italic">"Tri thức là sức mạnh, và sự tự học là chìa khóa mở cánh cửa tri thức."</p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {filteredResources.map((res, i) => (
              <motion.a
                ...
                animate={{ opacity: 1, y: 0 }}
                className="p-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm hover:shadow-md hover:border-[#04AA6D] transition-all group flex flex-col"
              >
                ...
                <h3 className="text-xl font-black text-slate-800 dark:text-slate-100 mb-4 group-hover:text-[#04AA6D] transition-colors">{res.title}</h3>
...
const SkillsPage = ({ searchQuery }: { searchQuery: string }) => {
  ...
  return (
    <div className="pt-20 flex min-h-screen bg-slate-50 dark:bg-slate-950">
      {/* Sidebar - W3Schools Course Tracks */}
      <aside className="w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 hidden lg:block sticky top-20 h-[calc(100vh-80px)] overflow-y-auto">
        <div className="p-8">
                ...
                {courses.map(course => (
                  ...
                  >
                    <div className="relative aspect-video overflow-hidden">
                      ...
                      <div className="absolute top-4 right-4 bg-white/90 dark:bg-slate-800/90 backdrop-blur px-3 py-1 rounded-xl flex items-center gap-1 shadow-sm">
                        <Star size={12} className="text-amber-500 fill-amber-500" />
                        <span className="text-xs font-black">{course.rating}</span>
                      </div>
                    </div>
                    <div className="p-6 flex-1 flex flex-col">
                      <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2 group-hover:text-[#04AA6D] transition-colors line-clamp-2">{course.title}</h3>
...
const CourseDetailPage = ({ user }: { user: FirebaseUser | null }) => {
  ...
  return (
    <div className="pt-24 pb-20 px-4 md:px-6 max-w-[1600px] mx-auto">
      <div className="flex items-center gap-2 text-slate-400 font-bold mb-6 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer" onClick={() => navigate(-1)}>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 p-8 md:p-10 rounded-[40px] shadow-3d-sm border border-slate-100 dark:border-slate-800">
            ...
            <div className="grid md:grid-cols-2 gap-10">
              <div className="space-y-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 italic text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[40px] shadow-3d-sm border border-slate-100 dark:border-slate-800">
                  ...
                  className={cn(
                    ...
                    activeLessonIdx === idx 
                      ...
                      : completedLessons.includes(lesson.id)
                        ...
                        : "bg-white dark:bg-slate-800 border-slate-50 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 text-slate-600 dark:text-slate-300"
                  )}
                >
                  <div className={cn(
                    "w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm",
                    activeLessonIdx === idx ? "bg-white/20" : "bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-300"
                  )}>
...
const RewardsPage = ({ user }: { user: FirebaseUser | null }) => {
  ...
  return (
    <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
      <div className="text-center mb-16">
        <h1 className="text-5xl font-black dark:text-white mb-4">Cửa Hàng Đổi Quà</h1>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        {rewards.map((rw, i) => (
          ...
          >
            ...
            <button 
              disabled={!user}
              className="w-full py-4 bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-300 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-emerald-600 dark:hover:bg-emerald-500 hover:text-white disabled:opacity-50 transition-all active:scale-95"
            >
              Đổi Quà
            </button>
          </motion.div>
  ...
  return (
    <BrowserRouter>
      <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300">
        ...
        <footer className="bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 py-12 px-6">
          <div className="max-w-7xl mx-auto flex flex-col items-center justify-center gap-6">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-slate-900 dark:bg-emerald-600 rounded-xl flex items-center justify-center text-white">
                <BookOpen size={24} />
              </div>
              <p className="font-black dark:text-white">Self-Study Hub &copy; 2026</p>
            </div>
            <nav className="flex gap-8">
              <a href="#" className="text-sm font-bold text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">Điều Khoản</a>
              <a href="#" className="text-sm font-bold text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">Liên Hệ</a>
            </nav>
          </div>
        </footer>
